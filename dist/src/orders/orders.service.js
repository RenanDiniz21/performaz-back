...
let OrdersService = class OrdersService {
    ...
    async findAll(vendorId, clientId) {
        const query = this.db.select().from(schema.orders);
        const results = await query;
        return results.filter((o) => (!vendorId || o.vendorId === vendorId) &&
    ...
    async findOne(id) {
        const [order] = await this.db
            .select()
            .from(schema.orders)
            .where((0, drizzle_orm_1.eq)(schema.orders.id, id));
        if (!order)
            throw new common_1.NotFoundException("Pedido não encontrado");
        const items = await this.db
            .select()
            .from(schema.orderItems)
            .where((0, drizzle_orm_1.eq)(schema.orderItems.orderId, id));
        return { ...order, items };
    }
    async create(dto) {
        ...
        await this.db
            ...
            .where((0, drizzle_orm_1.eq)(schema.clients.id, dto.clientId));
        return { ...order, items };
    }
